...
if not _session_os.environ.get('VERIFICATION_SESSION_ROOT'):
    raise SystemExit(_session_subprocess.call(['python3', str(_SessionPath(__file__).resolve().parents[2] / 'scripts/verification-session.py'), 'mutations', *_session_sys.argv[1:]]))
...
('selection-overlay-keeps-interior-dim', 'SelectionOverlay.swift',
 'rect.fill(using: .clear)', 'rect.fill(using: .destinationOut)', 'SelectionOverlayContractTests.testSelectionRenderingDimsOutsideAndClearsInteriorWithoutDisplayingWindow'),
