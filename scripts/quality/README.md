...
Ordinary tests: `swift test`. Manual metrics use the installed Xcode SwiftParser and LLVM coverage tools, retaining the existing 90% line, 85% branch and CRAP <=8 gates. Missing Swift branch counters remain unverified. Manual coverage/metrics retain the existing exclusion of `EditorWindowControllerTests.testApplicationKeyDispatchRoutesUndoToVisibleEditor`, which dispatches to a visible window. Mutation selectors such as `--only NAME` or `--group followup` can follow `mutations`.
