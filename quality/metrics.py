...
report = dict(schema='zoomies-quality/v2', component='Zoomies app',
    ...
    summary=dict(function_bodies=len(functions), unmapped=len(functions)-len(scored),
                 max_complexity=max(f['complexity'] for f in functions), complexity_above_limit=sum(f['complexity']>CRAP_LIMIT for f in functions),
                 max_crap=max(f['crap'] for f in scored), crap_above_limit=sum(f['crap']>CRAP_LIMIT for f in scored)),
...
report['targets_met'] = (changed == [] and report['coverage']['lines']['percent'] >= 90 and branch_total > 0 and
                          ...
                          report['summary']['unmapped'] == 0 and report['summary']['crap_above_limit'] == 0)
